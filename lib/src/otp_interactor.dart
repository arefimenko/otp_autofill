...
class OTPInteractor {
  ...
  Future<String?> startListenUserConsent({String? senderPhone, VoidCallback? onSheetOpened}) async {
    if (_platform.isAndroid) {
      _channel.setMethodCallHandler((c) => _userConsentSheetOpen(c, onSheetOpened: onSheetOpened));
  ...
  Future<void> _userConsentSheetOpen(MethodCall call, {VoidCallback? onSheetOpened}) async {
    switch (call.method) {
      case 'userConsentAction':
        onSheetOpened?.call();
        break;
    }
  }
